...
for j, file in enumerate(filePaths):
    ...
    if generateStaticMaps:
        ...
        bmap.drawparallels(latTicks, labels=[1, 0, 0, 0], linewidth=0.0, fontsize=14)
        bmap.drawmeridians(lonTicks, labels=[0, 0, 0, 1], linewidth=0.0, fontsize=14)
        font = FontProperties()
        font.set_weight('bold')
        font.set_family('sans-serif')
        for i, town in enumerate(towns):
            plt.plot(townCoords[i][0], townCoords[i][1], 'ok', markersize=4)
            plt.text(townCoords[i][0], townCoords[i][1], town, fontproperties=font, fontsize=11)
